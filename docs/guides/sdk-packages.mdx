...
The two are tiers, not a migration path. The local engine covers single-process
execution on one machine. Reach for an external coordinator when you need
durability across processes, human decisions that span sessions, repeated
occurrences, or coordination across projects; see
