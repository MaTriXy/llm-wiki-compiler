...
`llmwiki workflow` is the lightweight tier: it runs one declared workflow in a
single process, on one machine, under the project lock, with no additional
infrastructure. It suits a single operator advancing a run from the terminal or
the SDK, and it remains supported on its existing experimental terms.
...
- runs that survive process restarts or move between machines
- human decisions that span sessions or involve several people
- repeated occurrences of the same process on a schedule or trigger
