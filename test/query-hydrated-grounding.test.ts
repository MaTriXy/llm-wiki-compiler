...
describe("query grounding identity is the HYDRATED set", () => {
  it("drops a selected-but-unreadable page from the returned grounding, with a warning", async () => {
  ...
  it("keeps every rendered chunk excerpt's parent inside the reported grounding", async () => {
    ...
    const result = await generateAnswer(root, "what are the chunky facts?", { debug: true });
    ...
    for (const parent of rendered) {
      expect(result.pageIds, `excerpt from ${parent} reached the model unreported`).toContain(parent);
    }
  });
});
