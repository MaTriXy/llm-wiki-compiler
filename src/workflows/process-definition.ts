/**
 * @file src/workflows/process-definition.ts
...
import { WorkflowProcessAuthorityError } from "./process-authority.js";
...
async function readDefinition(root: string, run: WorkflowRun): Promise<Record<string, unknown>> {
  ...
  if (member === undefined || member.digest !== run.processAuthority?.processDefinitionDigest) {
    throw new WorkflowProcessAuthorityError("stale-or-invalid");
  }
  const text = await readStoredMemberText(root, active.binding.packageDigest, member, MAX_PROCESS_DEFINITION_BYTES);
...
export async function resolveTerminalDisposition(
  ...
  const definition = await readDefinition(root, run);
