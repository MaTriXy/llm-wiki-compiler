/**
 * @file src/workflows/product-operation-output.ts
...
import {
  locatePreparationManifest, readPreparationRunForManifest,
} from "../preparations/service-run-lookup.js";
import { preparationManifestDigest, type PreparationManifestV1 } from "../preparations/manifest-parse.js";
import type { PreparationRunV1 } from "../preparations/run-types.js";
...
async function appliedTargets(root: string, expected: string, actual: string | undefined) {
  if (actual !== expected) refuse("preparation handoff does not match the submitted bundle");
  const observed = await observeOperationBundle(root, expected);
...
async function operationFacts(
  ...
  const authority = await verifiedPreparationAuthority(root, run, stage, output.preparationRunId);
  const targets = await appliedTargets(root, output.bundleManifestDigest, authority.preparation.handoff?.bundleManifestDigest);
...
async function verifiedPreparationAuthority(
  ...
  const located = await locatePreparationManifest(root, preparationRunId);
  if (!located.ok) refuse(`preparation did not locate (${located.reason})`);
  const lookup = await readPreparationRunForManifest(root, located.manifest);
...
export async function productPreparationRef(
  ...
  const { manifest, preparation } = await verifiedPreparationAuthority(root, run, stage, preparationRunId);
...
async function assertProductPreparationCurrent(
  ...
  const fresh = await productPreparationRef(root, run, stage, stored.preparationRunId);
...
export async function assertProductStageOutputCurrent(
  ...
  if (output?.kind === "product-operation") return assertProductOperationOutputCurrent(root, run, stage);
  ...
  return assertProductPreparationCurrent(root, run, stage, proof as unknown as ProductPreparationRefV1);
}
...
export async function recordProductOperationOutputLocked(
  ...
  const facts = await operationFacts(root, run, stage, output);
  const ref: ProductOperationRefV1 = { ...facts, settledAt: new Date().toISOString() };
  return { run: await recordSettledStageOutput(root, run, stage, { ...ref }), applied: true, decision: "accepted" };
}
...
export async function replayProductOperationOutputLocked(
  ...
  await assertProductOperationOutputCurrent(root, run, stage);
