/**
 * @file src/workflows/human-input-schema.ts
...
import type { EntityId, HumanInputDescriptorV1, HumanInputFieldV1, ProfilePack } from "../profile/types.js";
...
async function admitRefList(
  ...
  for (const member of value) {
    admitted.push(field.referenceKind === "entity"
      ? await admitEntity(root, profile, member, field, run, name)
      : await admitArtifact(root, profile, member, field, run, name));
  }
  return admitted;
}
...
async function admitField(
  ...
  if (field.kind === "entity-ref") return admitEntity(root, profile, value, field, run, name);
  if (field.kind === "artifact-ref") return admitArtifact(root, profile, value, field, run, name);
  return admitRefList(root, profile, run, field, value, name);
}
...
export async function admitHumanInput(
  ...
  for (const [name, field] of Object.entries(descriptor.fields)) {
    ...
    admitted[name] = await admitField(root, profile, run, field, value, name);
  }
  return admitted;
}
